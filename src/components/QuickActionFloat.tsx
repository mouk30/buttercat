...
export const QuickActionFloat: React.FC<QuickActionFloatProps> = ({ onOpenModal }) => {
  ...
  return (
    <div className="fixed bottom-5 right-4 sm:right-6 z-40 flex flex-col items-end gap-2.5">
      ...
      {/* Floating Free Diagnosis & Estimate CTA Button */}
      <button
        onClick={onOpenModal}
      ...
      >
        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-amber-50 border-2 border-amber-300 shadow-sm shrink-0">
          <img
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => {
            alert(
              `카카오톡 채널 [${AGENCY_INFO.kakaoId}] 로 연결됩니다.\n운영시간 동안 10분 이내 빠른 답변을 받으실 수 있습니다.`
            );
          }}
          className="w-11 h-11 rounded-full bg-[#FEE500] hover:bg-[#edd400] text-[#3C1E1E] shadow-md flex items-center justify-center transition-all duration-200 font-black text-xs cursor-pointer"
          title="카카오톡 실시간 상담"
        >
